...
- Rule-aware DNS routing: domestic DIRECT domains use AliDNS DoH directly for local CDN answers, while external DoH uses the failover-capable `极速` group; no system DNS upstream is used.
...
- Bilibili web, API, CNAME, image, and video CDN domains are pinned to DIRECT and excluded from fake-IP. The independent TUN uses MTU 1400; verification checks DNS routing, the proxy entry, and TUN access.
